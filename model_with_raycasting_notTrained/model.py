...
class QNet(nn.Module):
    def __init__(self, in_dim=20, hidden=512, out_dim=3):
        super().__init__()
...
class DQNAgent:
    def __init__(self, lr=1e-3, gamma=0.95,
                 eps_start=1.0, eps_min=0.01, eps_decay=0.99,
                 batch=128, target_update=200):
        self.device  = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Device: {self.device}")
        self.policy  = QNet().to(self.device)
    ...
    def learn(self):
        ...
        s, a, r, ns, done = (s.to(self.device), a.to(self.device),
                             r.to(self.device), ns.to(self.device), done.to(self.device))
        a = a.long().unsqueeze(1)

        q_cur  = self.policy(s).gather(1, a).squeeze()
        with torch.no_grad():
            best_actions = self.policy(ns).argmax(1).unsqueeze(1)
            q_next = self.target(ns).gather(1, best_actions).squeeze()
